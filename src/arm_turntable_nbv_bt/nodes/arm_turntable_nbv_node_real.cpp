...
#include "../include/arm_turntable_nbv_bt/Wait.h"
...
int main(int argc, char ** argv){
    ...
    ros::init(argc, argv, "arm_turntable_nbv_node");
    ...
    // add Wait node
    BT::NodeBuilder builder_Wait = [&root_nh, &bh_tree_nh](const std::string& name,
                                                                const BT::NodeConfiguration& config) {
        return std::make_unique<Turntable_NBV::Wait>(name, config, root_nh, bh_tree_nh);
    };
    factory.registerBuilder<Turntable_NBV::Wait>("Wait", builder_Wait);

    // construct tree 
    auto file_path = ros::package::getPath("arm_turntable_nbv_bt") + "/config/tree/ArmTurntableNBVReal.xml";
