...
void SeeMain(ros::NodeHandle &n) {
  ...
  while (ros::ok() && !core->IsDone()) {
    ...
    if (ReachedNBV(cloud_sub->view, core->GetNBV(), nbv_params, first_view)) {
      ...
      // 等待 benchmark 节点发送开始计算的信号
      bool start_trig = false;
      if (ros::param::has("/see/benchmark_start")) {
        ROS_INFO_STREAM("start_trig, has /see/benchmark_start");
      }
      while (!start_trig)
      {
        ros::param::get("/see/benchmark_start", start_trig);
        // 对当前线程增加一个短暂的休眠，避免线程占用过高
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        // 保持 ROS 的消息循环
        ros::spinOnce();
        ROS_INFO_STREAM("Waiting for trigger");
      }
      ...
      ros::param::set("/see/benchmark_start", false);
    }else
    {
      // 对当前线程增加一个短暂的休眠，避免线程占用过高
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
      // 保持 ROS 的消息循环
      ros::spinOnce();
      ROS_INFO_STREAM("Waiting for Move to NBV");
    }
