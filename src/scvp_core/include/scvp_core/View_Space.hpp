...
public:
	...
	void get_view_space(vector<Eigen::Vector3d>& points) {
		...
		cout << "predict size is " << predicted_size << endl;
		int sample_num = 0;
		int viewnum = 0;
		//第一个视点固定为模型中心
#ifdef ROS
		Eigen::Vector3d offset(0.9, 0, 0);
		Eigen::Vector3d new_position = offset + object_center_world;
    	View view(offset);
#else
...
#ifdef ROS
	View_Space(int _id, Share_Data* _share_data, Voxel_Information* _voxel_information, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,int _first_view_id, double _predicted_size) {
		predicted_size = _predicted_size;
		...
		if (share_data->method_of_IG == 7 || share_data->method_of_IG == 6 || share_data->method_of_IG == 8) {
			...
			if (fin.is_open()) { //存在文件就读视点集合
				...
				cout << "SCVP viewspace readed." << endl;
			}
			else {
			...
			if (fin.is_open()) { //存在文件就读视点集合
				...
				cout << "NBV viewspace readed." << endl;
			}
			else { //不存在就生成视点集合
				//获取点云BBX
				vector<Eigen::Vector3d> points;
		...
		//第一次的数据，根据BBX初始化地图
		double now_time = clock();
		//octo_model->setNodeValue(object_center_world(0), object_center_world(1), object_center_world(2), (float)0, true); //初始化概率0.5，即logodds为0
	...
	void add_bbx_to_cloud(pcl::visualization::PCLVisualizer::Ptr viewer) {
		std::cout << "predicted_size: " << predicted_size << endl;
