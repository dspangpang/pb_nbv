...
def eval(datapath):
    ...
    checkpoint = torch.load('./root/work_place/pb_nbv/src/scvp_core/sc-net/pt/last.pth.tar',map_location = torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    pred = eval(name_of_model)
    ans = []
    for i in range(pred.shape[1]):
        if pred[0][i] == 1:
            print(i)
            ans.append(i)
    np.savetxt(log_path + '/'+name_of_model+'.txt',ans,fmt='%d')
