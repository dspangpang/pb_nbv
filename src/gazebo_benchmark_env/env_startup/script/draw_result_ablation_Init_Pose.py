...
data_dir = "/root/work_place/pb_nbv/src/gazebo_benchmark_env/env_startup/res_data/ablation"
...
def process_data(path, group):
    ...
    for folder in folders:
        ...
        for obj_folder in obj_folders:
            ...
            for i in range(len(iter_folders)):
                # np 读取文件夹下的 nbv_pose.txt 文件 中的 4x4 矩阵
                pose_file_path = os.path.join(path, folder, obj_folder, iter_folders[i], 'nbv_pose.txt')
                with open(pose_file_path, 'r') as f:
                            content = f.read()
                            content = content.replace('[', '').replace(']', '')
                            content_io = io.StringIO(content)
                            pose = np.loadtxt(content_io)
                ...
                # 把distance 加到 写入文件
                with open(os.path.join(path, folder, obj_folder, iter_folders[i], 'distance.txt'), 'w') as f:
            ...
            # 把 total_distance 加到 写入文件
            with open(os.path.join(path, folder, obj_folder, 'total_distance.txt'), 'w') as f:
                f.write(str(total_distance))

            # 把最终的 coverage 写入文件
            with open(os.path.join(path, folder, obj_folder, 'coverage.txt'), 'w') as f:
                f.write(str(coverage_current[-1]))

            # 计算 coverage per distance
            coverage_per_distance = coverage_current[-1] / total_distance
            with open(os.path.join(path, folder, obj_folder, 'coverage_per_distance.txt'), 'w') as f:
                f.write(str(coverage_per_distance))

            total_coverage_per_distance.append(coverage_per_distance)
    ...
    # 新建一个 txt 文件 保存覆盖率 per distance的均值
    average_coverage_per_distance = 0
    for iter in total_coverage_per_distance:
        average_coverage_per_distance += iter / len(total_coverage_per_distance)
    with open(os.path.join(path, "figuredata", f"{group}_coverage_per_distance.txt"), 'w') as f:
        f.write(str(average_coverage_per_distance))
...
if __name__ == '__main__':
    ...
    groups = ["pb-4-10-0.9,0,0", "pb-4-10-0,0.9,0", "pb-4-10-0,0,0.9", "pb-4-10-0,0.636,0.636"]
